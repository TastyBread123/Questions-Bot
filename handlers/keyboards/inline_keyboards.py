...
BACK_KB = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='⬅️ Назад', callback_data='to_main_menu')]])
...
def start_contest_kb(contest_id: int):
    keyboard = [
            [InlineKeyboardButton(text='✅ Начать викторину', callback_data=f'start_enter_id-{contest_id}')],
            [InlineKeyboardButton(text='⬅️ Назад', callback_data='to_main_menu')]
        ]

    return InlineKeyboardMarkup(inline_keyboard=keyboard)

def restart_contest_kb(contest_id: int):
    keyboard = [
            [InlineKeyboardButton(text='🔄 Перепройти викторину', callback_data=f'start_enter_id-{contest_id}')],
            [InlineKeyboardButton(text='⬅️ Назад', callback_data='to_main_menu')]
        ]
...
async def add_variants_kb(question_id: int, variants: list = None):
    kb = [
            [InlineKeyboardButton(text='✅ Дальше', callback_data=f'finish_variant_{question_id}')]
        ]
...
async def results_kb(contest_id: int, results: dict):
    kb = [
            [InlineKeyboardButton(text='✅ Дальше', callback_data=f'finish_results_{contest_id}')]
        ]
