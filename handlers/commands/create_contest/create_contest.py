...
from handlers.keyboards import BACK_KB, BACK_TO_MAIN_MENU, FINISH_CREATE_CONTEST_KB, SKIP_DOP_TEXT_CREATE, add_qustions_kb, results_kb
...
async def contests_create_callback(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("💥 Введите название викторины <i>(до 32 символов)</i>:", reply_markup=BACK_KB, parse_mode='HTML')
...
async def choose_title_create_contest(message: types.Message, state: FSMContext):
    ...
    await message.answer("💥 Введите описание викторины <i>(до 300 символов)</i>:", parse_mode='HTML')
